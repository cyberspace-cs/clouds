...
description: |
  中文名：性能优化。优化前后端、查询与数据库性能：Core Web Vitals、加载时间、N+1 查询、性能回归、profiling 瓶颈。
  English: Optimizes application performance across frontend, backend, queries, and databases. Use when performance requirements exist, when you suspect performance regressions, when Core Web Vitals or load times need improvement, when N+1 query patterns need fixing, or when profiling reveals bottlenecks.
---
